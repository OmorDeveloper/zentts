...
def check_required_files (model_path ="ZENTTS-v1.0.onnx",voices_path ="voices-v1.0.bin"):
    ...
    required_files ={
    model_path :"https://github.com/nazdridoy/ZENTTS-tts/releases/download/v1.0.0/ZENTTS-v1.0.onnx",
    voices_path :"https://github.com/nazdridoy/ZENTTS-tts/releases/download/v1.0.0/voices-v1.0.bin"
    }
    ...
    if missing_files :
        ...
        print (f"\nPlace the downloaded files in the same directory where you run the `ZENTTS-tts` command.")
...
def validate_language (lang ,engine ):
    ...
    try :
        supported_languages =set (engine .get_languages ())# Get supported languages from ZENTTS
...
def print_usage ():
    print ("""
Usage: ZENTTS-tts <input_text_file> [<output_audio_file>] [options]
...
Examples:
    ...
    ZENTTS-tts input.pdf output.wav --speed 1.2 --lang en-us --voice af_sarah
    ...
    ZENTTS-tts input.txt --stream --voice "am_adam,af_sarah" # 50-50 blend
    ...
    ZENTTS-tts input.txt output.wav --model /path/to/model.onnx --voices /path/to/voices.bin
    ZENTTS-tts input.txt --model ./models/ZENTTS-v1.0.onnx --voices ./models/voices-v1.0.bin
    """)

def print_supported_languages (model_path ="ZENTTS-v1.0.onnx",voices_path ="voices-v1.0.bin"):
    """Print all supported languages from ZENTTS."""
    check_required_files (model_path ,voices_path )
    try :
        engine =Kokoro (model_path ,voices_path )
        languages =sorted (engine .get_languages ())
        print ("\nSupported languages:")
...
def print_supported_voices (model_path ="ZENTTS-v1.0.onnx",voices_path ="voices-v1.0.bin"):
    """Print all supported voices from ZENTTS."""
    check_required_files (model_path ,voices_path )
    try :
        engine =Kokoro (model_path ,voices_path )
...
def validate_voice (voice ,engine ):
    ...
    """
    try :
        supported_voices =set (engine .get_voices ())
...
def process_chunk_sequential (chunk :str ,engine: Kokoro ,voice :str ,speed :float ,lang :str ,
    ...
    try :
        ...
        if "index 510 is out of bounds"in error_msg :
            ...
            for i ,piece in enumerate (pieces ,1 ):
                ...
                samples ,sr =process_chunk_sequential (piece ,engine ,voice ,speed ,lang ,
                retry_count +1 ,debug )
...
model_path ="ZENTTS-v1.0.onnx",voices_path ="voices-v1.0.bin"):
    ...
    if stream :
        ...
        for chapter in chapters :
            ...
            asyncio .run (stream_audio (engine ,chapter ['content'],voice ,speed ,lang ,debug ))
    else :
        if split_output :
            os .makedirs (split_output ,exist_ok =True )
...
def main ():
    """Main entry point for the ZENTTS-tts CLI tool."""
    ...
    if '--version'in sys .argv or '-v'in sys .argv :
        try :
            print (f"ZENTTS-tts version {importlib.metadata.version('ZENTTS-tts')}")
        except importlib .metadata .PackageNotFoundError :
            print ("ZENTTS-tts version unknown (not installed)")
        sys .exit (0 )
    ...
    # For help commands, we need to parse model/voices paths first
        model_path ="ZENTTS-v1.0.onnx"# default model path
